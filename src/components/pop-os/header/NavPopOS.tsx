...
const NavPopOS = () => {
  ...
  return (
      <div className="ml-4 flex gap-6 text-gray-400 text-lg">
        <NavLink to="/" className={({isActive}: any) => (isActive ? 'border-b-2 border-gray-200 text-gray-200' : 'border-b-2 border-transparent') + ' flex items-center'}>
        ...
        <NavLink to="/my-anime" className={({isActive}: any) => (isActive ? 'border-b-2 border-gray-200 text-gray-200' : ' border-b-2 border-transparent') + ' hidden lg:flex items-center'}>
          <span className="flex items-center gap-1 px-2 py-0.5 hover:bg-gray-200 hover:bg-opacity-20 hover:text-gray-200 rounded-lg transition ease-in-out">
            🍥 My anime
          </span>
        </NavLink>
